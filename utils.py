...
def makeMoleculeList(moleculeFile):
	mlist=[]
	#mlist.append['<unkown>']
	with open(moleculeFile,'r',encoding='utf-8') as f:
		for line in f:
...
def makePOSList(lista):
	sent_list = sum(lista, [])#将列表中的所有元素变为一个list
	wf = {}
	for sent in sent_list:
		for w in sent:
			if w in wf:
...
def mapDrugToId(drug_contents, drug_list):
	T = []
	for drug in drug_contents:
		T.append([drug_list.index(drug)])
	#print(T)
	#print(len(T))
	return np.array(T)
...
def dataRead(fname):
	...
	fp = open(fname, 'r')
	samples = fp.read().strip().split('\n\n')#every sample include the text ,drug pair and DDI
...
#			continue
		e1, e1_t, e2, e2_t = entities.split('\t')#e1:name;e1_type:drug_n or group 
		sent_contents.append(sent.lower())
		entity1_list.append([e1.lower(), e1_t])
		entity2_list.append([e2.lower(), e2_t])
		sent_lables.append(relation)
...
#		
		...
		entity1 = ent1[0].lower()
		entity2 = ent2[0].lower()
		molecule_list1.append(entity1)
		molecule_list2.append(entity2)
	...
	return word_list, d1_list, d2_list, type_list,molecule_list1,molecule_list2
...
def readMoleculeEmb(molecule_list, fname, embSize=100):
	print ("Reading molecule vectors")
	...
	embeddings = np.asarray(wv)
    #print(embeddings)
	embeddings=np.float32(embeddings)
	

	print ("number of molecule embedding", len(wv))
	return embeddings
