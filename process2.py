...
train = open('hierarchical_DDI2013_preprocess/test_data_cut.txt','r')

fw = open('hierarchical_DDI2013_preprocess/test_data_cut_processd2.txt','w')

def preProcess(sent):
	sent = sent.replace(',',' ,')
	sent = sent.replace('-',' ')
	sent = sent.replace('/',' / ')
	sent = re.sub('([0-9]{1,}[.][0-9]*)', "dga", sent)#正则数字替换为dg
	sent = re.sub('([0-9]{1,})', "dgb", sent)#正则数字替换为dg
